...
```{r Gene annotation exploration}
dim(gene.descriptions)
count.number.gene<-nrow(gene.descriptions)
gene.descriptions.lenghth <- vector()
j<- 1
for (j in 1:nrow(gene.descriptions)){
 
    if (!isEmpty(gene.descriptions[j,6]) && !isEmpty(gene.descriptions[j,5]) ){
 gene.descriptions.lenghth<- gene.descriptions[j,6] - gene.descriptions[j,5] 
  }

}


gene.descriptions <-as.matrix(gene.descriptions)


gene.leghth<-apply(gene.descriptions[,], 1,length)
gene.descriptions.modifed<- data.frame( matrix(ncol =ncol(gene.descriptions), nrow= nrow(gene.descriptions)),
              names(gene.descriptions.modifed)<-names(gene.descriptions),                 
              row.names(gene.descriptions.modifed)<-row.names(gene.descriptions)
                                        )

View(gene.descriptions.modifed)
```
