...
# Count the nu;ber of genes from the RNA-seq data for which we have no info in the gene description file.
...
```{r yeast_gene_length_distrib, fig.width=8, fig.height=6, fig.cap="Length distribution for all the yeast genes", fig.path="figures/"}
# 1. Count the number of genes in the `gene.descriptions` data frame.
...
mean(gene.descriptions$length) 
(gene.descriptions$length)

  
# 6. Based on the column `chrom.id`, count the number of reads per chromosome (use the function `table()`), and represent it in a barplot.
x <-table(gene.descriptions$chrom.id)
View(x)
# 7. Using the R function `grep()`, select all the genes whose name (`name`column of the gene.description data frame) contains the string "SNF".

 grep("SNF",names(gene.descriptions$name))
```
