...
Based on the standardized log2-transformed counts, compute a table with statistics per gene: for each gene, compute the mean, sd, var, min, median, max, IQR, and a few selected quantiles (0, 0.01, 0.05, 0.10, 0.25, 0.50, 0.75, 0.95, 0.99, 1), ...
...
median.per.gene<- apply(counts, 1, median)
median.count <- median(unlist(counts))

## Compute the median-normalized counts
counts.epsilon [1:5,1:5]
counts.mednorm <- counts.epsilon

for (j in 1:ncol(counts.mednorm)) {
  counts.mednorm[,j] <-   counts.epsilon[,j] / median.per.gene[j] * median.count
}
...
rownames(gene.quantiles) <- paste(sep="", "Q", sprintf("%.2f",selected.quantiles))
