...
plot(mean.per.gene.arithm, mean.per.gene.geom, log="xy", col="#BBBBBB", pch=".")
grid()
...
plot(mean.per.gene.arithm, mean.per.gene.rel.diff, log="xy")
...
## Compute the size factor for each sample (column)
j <- 1
for (j in 1:ncol(counts.all)){
  gm[j] <- gm_mean(counts.all[,j])
  s[j] <- median(counts.all[,j]/ gm[j]) 
}

j<-1
for(j in 1:ncol(counts.all)){
  x[j] <-counts.all[,j]/s[j]
}
qip <- 1/ncol(counts.all)* sum(x[j])

j <- 1
for(j in 1:ncol(counts.all)){
  wip[j] <- 1/(ncol(counts.all)-1) * sum((counts.all[,j]/s[j]-qip)^2)
  wip <- 1/(ncol(counts.all)-1) * sum((counts.all[,j]/s[j]-qip)^2)
}
 
j<-1
for(j in 1:ncol(counts.all)){
  z[j] <-1/s[j]
   zip <- (qip/ncol(counts.all))* sum(z)
