...
message("Loading counts from directory ", dir.counts)
...
gene.descr.file <- file.path(dir.counts, "gene_descriptions.tab")
message("Loading gene descriptions ", gene.descr.file)


gene.descriptions<- read.delim(gene.descr.file, header=TRUE, na.strings = "<NULL>")
...
## Gene annotation exploration

**Exercise for Mustafa: ** 

1. Count the number of genes in the `gene.descriptions` data frame.
2. Add a column to this data frame with the length of each gene. 
3. Draw an histogram with the distribution of gene lengths. 
4. Draw a box plot with the distribution of gene lengths. 
5. Compute the min, max, mean, median of all the gene lengths. 
6. Based on the column `chrom.id`, count the number of reads per chromosome (use the function `table()`), and represent it in a barplot.
7. Using the R function `grep()`, select all the genes whose name (`name`column of the gene.description data frame) contains the string "SNF".

```{r yeast_gene_length_distrib, fig.width=8, fig.height=10, fig.cap="Length distribution for all the yeast genes", fig.path="figures/"}
# 1. Count the number of genes in the `gene.descriptions` data frame.
#dim(gene.descriptions)
count.number.gene<-nrow(gene.descriptions)
message(sep= '   ' ,"Number of genes"," =  ", count.number.gene  )
# Count the number of genes from the RNA-seq data for which we have no info in the gene description file.
message("Genes with coordinates in the description file: ", sum(!is.na(gene.descriptions$start)))
message("Genes with no coordinates in the description file: ", sum(is.na(gene.descriptions$start)))


# 2. Add a column to this data frame with the length of each gene. 
gene.descriptions$length <- gene.descriptions$end - gene.descriptions$start + 1

# 3. Draw an histogram with the distribution of gene lengths. 
par(mfrow=c(2,1))

max.gene.len <- max(gene.descriptions$length, na.rm = TRUE)
break.size <- 100
hist(gene.descriptions$length, 
      col="#FFDDAA", #  ylim=c(0,300),
     main="Gene length distribution",
     breaks=seq(from=0, to = break.size*(ceiling(max.gene.len/break.size)), by=break.size),
     xlab="Gene length",
     ylab="Number of genes"
  )
abline(v=mean(gene.descriptions$length, na.rm = TRUE), col="brown", lwd=3)
abline(v=median(gene.descriptions$length, na.rm = TRUE), col="darkgreen", lwd=3)
abline(v=quantile(x = gene.descriptions$length, na.rm = TRUE, probs=0.25), col="darkgreen", lwd=1) ## Plot the first quartile
abline(v=quantile(x = gene.descriptions$length, na.rm = TRUE, probs=0.75), col="darkgreen", lwd=1) ## Plot the third quartile

# 4. Draw a box plot with the distribution of gene lengths. 
boxplot( gene.descriptions$length,
    horizontal = TRUE, las=1, col="#EEDDFF",
    main="The length for each genes in yeast Data Sets  ", cex.axis=0.6, cex.label=0.8,
    xlab=paste(sep="", " The Boxplot the distribution of gene lengths" ))  
...
- On the histogram, we see that there is a difference between the mean (brown vertical bar) and the median (thick dark green vertical bar). How do you explain this ? 

* The Median is the value that spearate tha higher half of Data from lower half, thence we use the Median in order to Descibing data compare with Mean which is simply described as the "Average" , furhter more the meadian is simply not affected by extremaly large or small values, this mean it is not skewed , but the Mean may be skewed by a small number extremely high or low values. briefly: For symmetric distributions, the mean is approximately equal to the median.  The tails of the distribution are the parts to the left and to the right, away from the mean. The tail is the part where the counts in the histogram become smaller.  
...
# 5. Compute the min, max, mean, median of all the gene lengths.
summary(gene.descriptions$length)
# which.max(gene.descriptions$length)
# which.min(gene.descriptions$length)
# sum(gene.descriptions$length)

gene.len.stats <- data.frame(
  min = min(gene.descriptions$length, na.rm=TRUE),
  Q1 = quantile(x=gene.descriptions$length, probs=0.25, na.rm=TRUE),
  median = median(gene.descriptions$length, na.rm=TRUE),
  mean = mean(gene.descriptions$length, na.rm=TRUE),
  Q3 = quantile(x=gene.descriptions$length, probs=0.75, na.rm=TRUE),
  max = max(gene.descriptions$length, na.rm=TRUE)
)
kable(gene.len.stats, row.names = FALSE)

```

### Number of genes per chromosome

```{r genes_per_chromosome}
# 6. Based on the column `chrom.id`, count the number of reads per chromosome (use the function `table()`), and represent it in a barplot.
x <- data.frame(table(gene.descriptions$chrom.id))
names(x) <- c("Chromosome", "Number of genes")
kable(x, caption = "**Table:** Number of genes per chromosome in the yeast *Saccharomyces cerevisiae*. ")
rm (x)
```

### Description of the SNF genes


```{r snf_genes}

# 7. Using the R function `grep()`, select all the genes whose name (`name`column of the gene.description data frame) contains the string "SNF".
SNF.gene.indices <- grep(pattern = "SNF", x = gene.descriptions$name, value = FALSE)
SNF.gene.names <- grep(pattern = "SNF", x = gene.descriptions$name, value = TRUE)

## Print an R-markdown formatted table with the description of the SNF genes
gene.descriptions$synonyms <- gsub(pattern = ";" , replacement = "; ", x = gene.descriptions$synonyms)

kable(gene.descriptions[SNF.gene.indices, ], row.names = FALSE, 
      caption = "**Table: SNF gene annotations.** ")
...
counts.all <- cbind(counts.wt, counts.snf2)
dim(counts.all)
...
| All | `r nrow(counts.all)` | `r ncol(counts.all)` |
| | |
...
## summary(counts.all) ## This takes too much place for a report
```
...
undetected.genes.per.sample <- apply(counts.all == 0, 2, sum)

## Count the number of samples where each gene has not been detected
message("Counting undetected samples per gene.")
undetected.samples.per.gene <- apply(counts.all == 0, 1, sum)
hist(undetected.samples.per.gene, 
     breaks=0:ncol(counts.all), col="grey", ylim=c(0,300),
...
for (j in 1:ncol(counts.all)){
  message("PROBLEM: no ggplot for sample nb ", j)
   # ggplot(counts.all, aes(x = counts.all[,j])) + geom_histogram(fill = "#525252", binwidth = 2000)
}
...
## sum(counts.all == 0) ## Count the number of zero values
counts.epsilon <- counts.all
counts.epsilon[counts.all == 0] <- epsilon
...
We dispose of a raw count table containing `r nrow(counts.all)` rows (one row per gene) and `r ncol(counts.all)` columns (one column per sample). 
...
k.prime <- which(names(Y) == ref.sample) ## Indexof the reference sample (k' in Robinson-Oshlack paper)
...
We will generate various plots to compare the expression level of an arbitrary sample (e.g. sample SNF2.2) with the reference sample (in this case, `r ref.sample`).

```{r MA-plot, fig.width=12, fig.height=12, fig.cap="Count comparisons between 2 arbitrary samples. "}
test.sample <- "SNF2.2"

par(mfrow=c(2,2))

## Draw a scatter plot with the raw counts
max.count <- max(c(counts.all[, ref.sample], counts.all[, test.sample]))
plot(counts.all[, ref.sample], counts.all[, test.sample],
     main="Raw count comparison",
     xlab=ref.sample,
     ylab=test.sample,
     col="#888888", 
     xlim=c(0, max.count),
...
## Draw a scatter plot with the log2-transformed counts
max.log2.count <- max(c(counts.log2[, ref.sample], counts.log2[, test.sample]))
plot(counts.log2[, ref.sample], counts.log2[, test.sample],
     main="Log2-tranformed count comparison",
     xlab=ref.sample,
     ylab=test.sample,
     col="#888888", 
     xlim=c(0, max.log2.count),
     ylim=c(0, max.log2.count),
     grid(col="#BBBBBB", lty="solid"),
     cex=0.5)
abline(a=0, b=1, col="black")


## Draw an M-A plot for some arbitrary sample
plot(A[, test.sample], M[, test.sample],
main=paste ("MA plot:", test.sample, "vs", ref.sample),
     xlab="M = mean log2-transformed counts",
     ylab="A = Log2 fold change",
     col="#888888", 
     grid(col="#BBBBBB", lty="solid"),
     cex=0.5)
...
- The plot with raw counts shows very well that there is a handful of outlier genes, with huge count numbers (~100.000 counts for a single gene !), whereas most genes have much smaller counts (average=1440)?. 

- The log2-transformed counts better show the dynamic range of counts: the dots corresponding to the genes are better separated on the graph. 

- The MA plot can be *approximately* perceived as a "rotation" of the scatter plot of log2 counts. The abcsissa ($M$) indicates the mean level of expression (in log2-transformed counts) whereas the ordinate indicates the orientation of the impact of SNF2 mutation: above the axis for the genes that are up-regulated (expressed at a stronger level in the SNF2 mutant than in the WT), and below the axis for down-regulated genes.

- Since the normal function of the Snf2 protein is to repress expression, we could say that the genes that are up-regulated in SNF2 mutant are "de-repressed": the idea is that in normal conditions (in the WT strain) these genes are repressed by the Snf2 protein, but in the SNF2 mutant the protein is defective, so these genes are not repressed anymore. 

- In the next steps, we will apppy differential analysis in order to establish a list of all the genes that are de-repressed (down-regulated) in SNF2 mutant, and another list with all the genes that are up-regulated (expressed stronger) in SNF2 mutant relative to WT. 

- On the raw count plot, we see that there is a gene that is highly expressed in the wild-typpe (8e+4 reads) but very weak in the Snf2 mutant (~0 reads).

```{r TMM computation}
## Compute the TMM
trimming.percent <- c("lower"=3, "upper"=5) ## Parameters: upper and lower percent to trim
trimming.number <- round(nrow(M) * trimming.percent / 100) ## Compute the number of genes to trim according to the above percent

TMM <- vector()
TMM_median <- vector()

j <- 1 ## Not necessary, just for quick test without running the whole loop
for (j in 1:ncol(M)) {
...
## Compare trimmed mean and trimmed median
plot(TMM, TMM_median)
  
par(mfrow=c(1,2))
hist(TMM, breaks=100, col="cyan")
hist(TMM_median, breaks=100, col="yellow")
par(mfrow=c(1,1))
...
# The rlog Trasformation

when we need the amount of variance is approximately the same across diffrent mean values,in stady case for multidimentional dta for example clustring and principal components analysis(PCA), this considered common statistical methods for exploratory analysis of multidimentional data. these suppose the data genarally has the same range of variance at diffrent ranges of the mean values this data so-called *HOMOSKEDASTIC*.
...
# nrow(counts.all)
# colData(counts.all)
# dds<- counts.all[rowSums(counts.all)> 1,]
# 
# nrow(dds)
# dds<-t(dds)
# 
# rlcounts<-rlog(t(dds), blind=FALSE)
...
stats.per.sample <- data.frame(
  mean = apply(counts.all, 2, mean),
  sd = apply(counts.all, 2, sd),
  var = apply(counts.all, 2, var),
  min = apply(counts.all, 2, min),
  median = apply(counts.all, 2, median),
  max = apply(counts.all, 2, max),
  sum = apply(counts.all, 2, sum),
  IQR=apply(counts.all, 2, IQR)
)

selected.quantiles <- c(0,0.05,0.1,0.25,0.50,0.75,0.9,0.95,0.99, 1)
sample.quantiles <-  round(t(apply(counts.all, 2, quantile, selected.quantiles)), digits=1)
...
# ## counts
# hist(log10(apply(counts.all, 2, sum)), breaks=50,
...
# print(counts.all[1000:1010,])
# dim(counts.all) ## Check the dimensions of the count table. 
...
for (genotype in c("WT", "SNF2")) {
  barplot(apply(counts.all[,sample.description$genotype == genotype], 2, sum), 
          horiz = TRUE,
          las=1, cex.names = 0.6,
...
median.per.column <- apply(counts.all, 2, median)

## Compute the global median count for all the samples together
median.count.allsamplestogether <- median(unlist(counts.all))
#dim(counts.all)
...
for (genotype in c("WT", "SNF2")) {
  boxplot(
    ...
    horizontal = TRUE, las=1, col=color.code[genotype],
    main=genotype, cex.axis=0.6, cex.label=0.8, 
    xlab=paste(sep="", "median-based normalized log2(counts.all), with epsilon=", epsilon))
}
...
The histograms above can thus be understood as a mixture of `r nrow(counts.all)` (number of genes) times 2 (number of genotypes) independent distributions, each of which is represented by 48 observations (replicates). 
...
stats.per.gene <- data.frame(
  mean= apply(counts.all, 1, mean),
  sd= apply(counts.all, 1, sd),
  var= apply(counts.all, 1, var),
  min= apply(counts.all, 1, min),
  median= apply(counts.all, 1, median),
  max= apply(counts.all, 1, max),
  IQR= apply(counts.all, 1, IQR)
  )
...
gene.quantiles <-  round(t(apply(counts.all, 1, quantile, selected.quantiles)), digits=1)
dim(gene.quantiles)
...
```{r fig.width=7, fig.height=7, fig.cap="Comparison of mean read log2(counts.all) per gene between the two genotypes. Note that this drawing is very preliminary, since we did not yet normalize the libraries. It is just to get a rough idea. "}
...
dgList <- DGEList(counts=counts.all, genes = rownames(counts.all)) 
...
Raw.count.gene = kable(counts.all[1:10, ], caption = "Raw counts per gene in each sample.")
...
cpm <- data.frame(matrix(ncol=ncol(counts.all), nrow=nrow(counts.all))) ## Instantiate a CPM data frame with NA values
row.names(cpm) <- row.names(counts.all)
names(cpm) <- names(counts.all)
# View(cpm)

## Compute the normalized counts (CPM) for each sample (column)
for (j in 1:ncol(cpm) ){
  cpm[,j] <- 1e+6 * counts.all[,j] / sum(counts.all[,j])
}
...
```{r Implementation of Trimmed Mean of M-Values (TMM)}
...
Mg <-function(counts.all) log2((counts.wt/libsum.k.wt.all)/(counts.snf2/libsum.k.snf2.all))
Ag <- function (counts.all) 1/2*(log2((counts.wt/libsum.k.wt.all)*(counts.snf2/libsum.k.snf2.all)))

Mg <-apply(as.matrix(counts.all) ,2, Mg)
Ag <- apply(counts.all, 2, Ag)   
# plot(Mg,Ag)
...
## Draw a mean/variance plot using the edgeR::meanVarPlot() function.
## Note: edgeR uses a different normalization method than DESeq2, so the plot is different.
...
cds <- newCountDataSet(counts.all,sample.description$replicate)
summary(cds)
print(cds)
...
```{r}
## Plot "diespersion" (i.e. the square of the variation coefficient) 
...
In DESeq, the inference relies on the estimation of the typical relationship between the data's variance (the data dispersion) and their mean, where  ** the dispersion** can be understood as the square of the coefficient of Biological variation, so, if a gene's expression typically differs from replicate to replicate sample by 20%, thence gene's dispersion is 0.2 power 2 = 0.04.

We can note that the variance between counts is the sum of two components: the sample-to-sample variation and uncertinity in measuring a concentration by counting reads.

This implies for us to estimate shot noise and Poisson noise, the latter being the dominating noise source for lowly expressed genes, whereas the former dominates for highly expressed genes. The sum of both, shot noise and dispersion, is considered in the differential expression inference. $v$ of count vaule is modelled as <font color="red">... ???</font>

#### The variance 

<font color="red">Mustafa: I think there is a problem with the formula below. Please check it</font>
...
*******************************************************************
# Diffrential expression at gene level by the DESeq package 
 $S_{j}$ is size factor 
 $K_{ij}$ is the count value 
...
s <- vector()  ## Size factor for sample normalization
x <- vector()  ## 
z <- vector()  ## 
vp <- vector()
wip <- vector()
...
## Compute the "pseudo-reference sample", i.e. for each gene, take the geometric mean of all th e samples
# gm <- vector() ## Geometric mean of counts across samples for each gene
counts.plus.one <- (counts.all + 1) ## To circumvent the absorbing effect of zero counts, add 1 to each count (tricky but this is hat everybody does)
dim(counts.plus.one)
range(counts.plus.one)

## Compute the "pseudo-reference" sample, i.e. a vector with one value per gene, where the value is the geometrc mean of counts for each gene, across all the samples
mean.per.gene.geom <- apply(counts.plus.one, 1, GeomMean)
length(mean.per.gene.geom)
table(is.infinite(mean.per.gene.geom))


## Compute and compare the geometic and arithmetic mean counts per gene
mean.per.gene.arithm <- apply(counts.plus.one, 1, mean)
plot(mean.per.gene.arithm, mean.per.gene.geom, log="xy", col="#6688FF", pch=".",
     title("Arithmetic and Geometric Means"), 
     xlab = "Arithmatic Mean per Genes", 
     ylab = "Geometric Mean per Genes",
     panel.first=c(grid(),
                   abline(a=0, b=1, col="red", lwd=1)))
                   
## Compute the relative difference between geometric and arithmetic means
mean.per.gene.rel.diff <- (mean.per.gene.arithm - mean.per.gene.geom) / (mean.per.gene.arithm)
plot(mean.per.gene.arithm, mean.per.gene.rel.diff, 
     log="xy",
     title("The relative difference between Geometric and Arithmetic Means"), 
     xlab = "Arithmetic Mean per Genes ", 
     ylab = "The relatives Differences Between Arithmetic and Geometric Means" ) 


pseudoref.sample <- mean.per.gene.geom
genewise.normalized.counts <- counts.plus.one
for (c in 1:ncol(genewise.normalized.counts)) {
  genewise.normalized.counts[,c] <- genewise.normalized.counts[, c] / pseudoref.sample
}
size.factor <- apply(genewise.normalized.counts, 2, median)
barplot(size.factor, horiz = TRUE, las=1, cex.names =0.5,
        col=sample.description$color)

## On the barplot, we get the impressio that the WT size factors are generally smaller
## than the SNF2 size factors. We can test this with a mean comparison test.
## We cannot be sure that the data is normal, but since we have relatively large sample sizes (48 samples in each group) we coulduse the t test, since it is robust to non-normality by virtue of the central limit theorem. 
size.factor.WT <- size.factor[sample.description$genotype == "WT"]
size.factor.SNF2 <- size.factor[sample.description$genotype == "SNF2"]
mean(size.factor.WT)
mean(size.factor.SNF2)
t.test(x = size.factor.WT, y = size.factor.SNF2, var.equal=TRUE) ## Run Student t-test, assuming variance equality
t.test(x = size.factor.WT, y = size.factor.SNF2, var.equal=FALSE) ## Run Welch t-test, which does not assume variance equality
## Welch and Student t-tests lead to the same conclusions: for some reason, the mean size factors are significantly different between the WT and the SNF2 samples. We should come back to this later, in order to understand it. Would it be possible that the WT and SNF2 libraries had different sequencing depths?


## Revised up to here with Jacques on 2016-09-14

## Compute the normalized counts: for each sample (column), 
## divide the counts by the size factor.
c <- 1
counts.norm.deseq1 <- counts.plus.one
for (c in 1:ncol(counts.norm.deseq1)){
  counts.norm.deseq1[,c] <- counts.norm.deseq1[,c] / size.factor[c]
}
## Check the normalized counts with a boxplot
boxplot(log2(counts.plus.one), horizontal=TRUE, col=sample.description$color, las=1,
        main="Before normalization log2(k+1)")
boxplot(log2(counts.norm.deseq1), horizontal=TRUE, col=sample.description$color, las=1,
        main="After normalization log2((k+1)))/s)")

## Compute the mean counts per gene for each condition
mean.per.gene.deseq1 <- data.frame(
  WT = apply(counts.norm.deseq1[, sample.description$genotype=="WT"], 1, mean),
  SNF2 = apply(counts.norm.deseq1[, sample.description$genotype=="SNF2"], 1, mean)
)
## Plot a comparison of log2-transformed mean counts per gene beteen WT and SNF2 genotypes
plot(log2(mean.per.gene.deseq1), las=1,
     col="#BBBBBB",
     main="log2-transformed mean counts per gene",
     panel.first=c(grid()))
abline(a=0, b=1) ## Plot a diagonal
text(4,13, "over-expressed", pos=4)
text(11,3, "under-expressed", pos=4)


## Revised by JvH till here on 2016-09-20

c <- 1
for (c in 1:ncol(genewise.normalized.counts)){
  genewise.normalized.counts[,c] <- genewise.normalized.counts[,c] / size.factor
  abun.fragments.from.genes.under.condition <- genewise.normalized.counts[,c]/ ncol(genewise.normalized.counts)
}
  
## to estimate the Mean"qip", we an do the follwoing 

## to calculate the sample variances on common scale:
c <- 1
for (c in 1:ncol(genewise.normalized.counts)){
    genewise.normalized.counts[,c] <- ((genewise.normalized.counts[,c] / size.factor )- abun.fragments.from.genes.under.condition)^2
  sample.variance <- genewise.normalized.counts[,c]/ (ncol(genewise.normalized.counts)-1)
}

c <- 1
for (c in 1:ncol(genewise.normalized.counts)){
 zip <- (abun.fragments.from.genes.under.condition/ncol(genewise.normalized.counts))* (1/ size.factor[c])
}
...
# length(counts.all)
# x <-c(2,1,4)
# b <-prod(x,na.rm = TRUE)^(1/5)
# 
# m <-length(counts.all)
# prod(counts.all[,i])^(1/m), na.rm = TRUE)
# 
# for (i in 1:ncol(counts.all)){
#   x[i]<- apply(counts.all, 2,  )
# }

# for(i in 1:ncol(counts.all)){
 # k_iv<- apply(counts.all, 2, mean(log(counts.all)), na.rm = TRUE)   
# }
```
