...
[@Fang:2012fg] compares different statistical models and tests (Poisson, Fisher, negative binomial, ...) for the detection of differentially expressed genes. 
...
## compute quantile  
apply(counts, 2, quantile, c(0,0.05,0.1,0.25,0.50,0.75,0.9,0.95,0.99, 1))
## sum of counts in sample  Introduction to differential gene expression analysis using RNA-seq
apply(counts, 2, sum) 
## Median count per sample 
apply(counts, 2, median )
## estimate of inter-Quartile Range (IQR)
apply(counts, 2, FUN = IQR)
##  Classical statistical estimates
summary(counts)
...
=======
## sum of count in log2-transformed count per gen
apply(counts.log2, 1,sum )

## Median of count in log2-transformed count per gene
apply(counts.log2, 1,median)

## percintile of Rows in log2-transformed count per gene
apply(counts.log2, 1, quantile, c(0,0.05,0.1,0.25,0.50,0.75,0.9,0.95, 0.99, 1 ))

## estimete inter-Qurtile Range in log-trasformed count per gene
apply(counts.log2, 1, FUN = IQR)

## classical Statisticla estimate 
summary(counts.log2)
>>>>>>> e7dbfb91d661dc71f75dd62e4063b282cc0dc3f0
```
