...
max.count <- max(c(counts[, ref.sample], counts[, test.sample]))
plot(counts[, ref.sample], counts[, test.sample],
...
j <- 1
for (j in 1:ncol(counts)){
  gm[j] <-gm_mean(counts[,j])
  s[j] <- median(counts[,j]/ gm[j])
}
...
qip <- 1/ncol(counts)* sum(x[j])

j <- 1

for(j in 1:ncol(counts)){
  wip[j] <- 1/(ncol(counts)-1) * sum((counts[,j]/s[j]-qip)^2)

  wip <- 1/(ncol(counts)-1) * sum((counts[,j]/s[j]-qip)^2)
...
# for(i in 1:ncol(counts)){
 # k_iv<- apply(counts, 2, mean(log(counts)), na.rm = TRUE)   
# }
```
