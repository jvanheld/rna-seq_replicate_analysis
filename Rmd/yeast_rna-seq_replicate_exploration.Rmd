...
##  exp(sum(log(x[x > 0]), na.rm=na.rm) / length(x)) ## Attention, this formula is from internet and it is false, because Zero values are discarded !!!
  ...
  if (sum(x < 0) > 0) {
    warning("Cannot compute geometric mean for a vector with negative values. ")
    return(NA)
  }
...
mean.per.gene.geom <- apply(counts.plus.one, 1, gm_mean)
...
plot(mean.per.gene.arithm, mean.per.gene.geom, log="xy", col="#BACCFF", pch=".", title("Arithmetic and Geometric Means"), xlab = "Arithmatic Mean per Genes", ylab = "Geometric Mean per Genes" )
grid()
...
plot(mean.per.gene.arithm, mean.per.gene.rel.diff, log="xy",title("The relative difference between Geometric and Arithmetic Means"), xlab = "Arithmetic Mean per Genes ", ylab = "The relatives Differences Between Arithmetic and Geometric Means" ) 
