...
The total RNA dataset "Production",cannot be estimated directly, since we don't know the expression levels and true lengths of every gene.
...
for the sequencing data the gene-wise log-fold-changes as:
...
$$$C_{j}= /frac{10}{D_{j}}$
```{r Implemente- count per million normalization method }

D <- apply(counts, 2 , sum)
D <-sum(D)
for ( j in 1:ncol(counts) ){
  cpm <-(counts/D)*1000000
}
