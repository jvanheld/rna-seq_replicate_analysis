...
## we will now create a DGEList object to hold our read counts.
## DGEList is an example of the custom task-specific sructure that are frequently used in Bioconductor to make analyses easier 
...
# Now we will create variaty in filtring process.
##In our dataset we have approximatly 7125 Genes. However, many of them will not be expressed, or will not be repersented by enough reads to contribute to the analysis. Removing these genes means that we ulimately have fewer tests to perform, thereby we will reducing the problems associated with multiple testing by using cpm= Counts per million, whereas we will retain only those genes that are repersented at least 1 cpm reads in our two groups for gene ""WT and "SNF2".
...
```{r}
plotMDS(dgList,
        main= "Exmining inter samlple relationship",
        xlab = "Number of sample  within genes",
        ylab = "Number of genes through Samples"
        )
```
...
counts$row.names
counts$names
designMat <- model.matrix(~sample.description$replicate + sample.description$genotype) ### this need to Check with prof. Jacques.
designMat
```

# Estimating Dispersions 
Now, we will estimate the dispersion parameter for our negativee Bionomial Model.
